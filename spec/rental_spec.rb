...
describe Rental do
  ...
  context "Given a rental object" do
    it "generate a hash representation from it" do
      ...
      expect(hash['date'.to_sym]).to eql(@rental.date)
      expect(hash['person_id'.to_sym]).to eql(@rental.person.id)
      expect(hash['book_id'.to_sym]).to eql(@rental.book.id)
    end
  end
end
